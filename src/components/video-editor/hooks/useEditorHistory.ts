...
export function useEditorHistory({ buildSnapshot, applySnapshot }: UseEditorHistoryParams) {
	...
	const syncVersion = useCallback(() => setVersion((v) => v + 1), []);

	const canUndo = useMemo(() => pastRef.current.length > 0, [version]); // eslint-disable-line react-hooks/exhaustive-deps
	const canRedo = useMemo(() => futureRef.current.length > 0, [version]); // eslint-disable-line react-hooks/exhaustive-deps
