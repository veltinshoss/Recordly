...
export function useEditorAudioSync({
	...
	useEffect(() => {
		return () => {
			...
			for (const revoke of audioElementRevokersRef.current.values()) revoke();
			...
			for (const revoke of sourceAudioElementRevokersRef.current.values()) revoke();
	...
	useEffect(() => {
		...
		const activeSpeedRegion = speedRegions.find(
			(r) => currentTime * 1000 >= r.startMs && currentTime * 1000 < r.endMs,
		);
		const targetPlaybackRate = activeSpeedRegion ? activeSpeedRegion.speed : 1;
		...
		for (const audio of sourceAudioElementsRef.current.values()) {
			...
			if (Math.abs(audio.playbackRate - syncedRate) > 0.001) audio.playbackRate = syncedRate;
