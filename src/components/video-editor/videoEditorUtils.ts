...
import type { ExportEncodingMode, ExportMp4FrameRate, ExportPipelineModel, ExportQuality } from "@/lib/exporter";
import { FrameRenderer } from "@/lib/exporter";
import type { ExportBackendPreference } from "@/lib/exporter";
...
export function getSmokeExportConfig(search: string): SmokeExportConfig {
	...
	return {
		...
		encodingMode:
			enabled && params.get("smokeEncodingMode") === "fast" ? "fast"
			: enabled && params.get("smokeEncodingMode") === "balanced" ? "balanced"
			: enabled && params.get("smokeEncodingMode") === "quality" ? "quality"
			: undefined,
		shadowIntensity: enabled ? parseSmokeExportNonNegativeNumber(params.get("smokeShadowIntensity")) : undefined,
		webcamInputPath: enabled ? params.get("smokeWebcamInput") : null,
		webcamShadow: enabled ? parseSmokeExportNonNegativeNumber(params.get("smokeWebcamShadow")) : undefined,
		webcamSize: enabled ? parseSmokeExportNonNegativeNumber(params.get("smokeWebcamSize")) : undefined,
		pipelineModel:
			enabled && params.get("smokePipelineModel") === "modern" ? "modern"
			: enabled && params.get("smokePipelineModel") === "legacy" ? "legacy"
			: undefined,
		backendPreference:
			enabled && params.get("smokeBackendPreference") === "auto" ? "auto"
			: enabled && params.get("smokeBackendPreference") === "webcodecs" ? "webcodecs"
			: enabled && params.get("smokeBackendPreference") === "breeze" ? "breeze"
			: undefined,
...
export function calculateMp4SourceDimensions(
	...
	const sourceAR = safeH > 0 ? safeW / safeH : 16 / 9;
	const ar = getAspectRatioValue(aspectRatio, sourceAR);
	if (aspectRatio === "native") return { width: safeW, height: safeH };
	if (ar === 1) {
		const base = Math.max(2, Math.floor(Math.min(safeW, safeH) / 2) * 2);
		return { width: base, height: base };
	}
	if (ar > 1) {
		for (let w = safeW; w >= 100; w -= 2) {
			const h = Math.round(w / ar);
			if (h % 2 === 0 && Math.abs(w / h - ar) < 0.0001) return { width: w, height: h };
		}
		return { width: safeW, height: Math.max(2, Math.floor(safeW / ar / 2) * 2) };
	}
	for (let h = safeH; h >= 100; h -= 2) {
		const w = Math.round(h * ar);
		if (w % 2 === 0 && Math.abs(w / h - ar) < 0.0001) return { width: w, height: h };
	}
	return { height: safeH, width: Math.max(2, Math.floor((safeH * ar) / 2) * 2) };
}
...
export function calculateMp4ExportDimensions(
	...
	if (quality === "source") {
		return { width: Math.max(2, Math.floor(baseWidth / 2) * 2), height: Math.max(2, Math.floor(baseHeight / 2) * 2) };
	}
...
export async function writeSmokeExportReport(
	...
	try {
		...
		await window.electronAPI.writeExportedVideoToPath(buf, `${outputPath}.report.json`);
	} catch (err) {
		console.error("[smoke-export] Failed to write report", err);
	}
}
